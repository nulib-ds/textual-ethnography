# Canopy Template (Northwestern)

Northwestern University Libraries' branded starter for Canopy IIIF projects. When you need deeper documentation, examples, or release notes, visit the main project at https://nulib-ds.github.io/canopy/.

See `DESIGN.md` in this repository for the current state of Northwestern-specific layout, theme, and styling decisions.
...
Working inside `nulib-ds/canopy`? Run `npm run preview:template-northwestern` to stage this starter into `.template-northwestern-preview/`, install its dependencies, and launch `npm run dev`. Stop the script with `Ctrl+C` when you are done.
